...
/* ── Visualizer param labels: add breathing room ── */
.vis-param-label {
    font-family: 'Rajdhani', sans-serif;
    font-size: 0.82rem;
    color: #b39ddb;
...
with tab1:
    ...
    with col_right:
        if predict_btn:
            ...
            # ── All Models: single flex HTML block for perfect alignment ──
            st.markdown("##### All Models")
            cards_html = '<div style="display:flex;gap:10px;margin-top:0.5rem;">'
            for mname, mres in model_results.items():
                m = mres["model"]
                p = class_names[m.predict(input_scaled)[0]]
                cards_html += f"""
                <div style="flex:1;min-width:0;background:rgba(50,10,100,0.45);
                     backdrop-filter:blur(10px);-webkit-backdrop-filter:blur(10px);
                     border:1px solid rgba(167,139,250,0.3);border-radius:12px;
                     padding:0.9rem 0.4rem;text-align:center;box-sizing:border-box;">
                    <div style="font-size:0.58rem;font-family:Orbitron;color:#4fc3f7;
                         letter-spacing:0.08em;overflow:hidden;text-overflow:ellipsis;
                         white-space:nowrap;padding:0 4px;">{mname}</div>
                    <div style="font-size:1.6rem;margin:0.35rem 0;">{icons.get(p,'🔭')}</div>
                    <div style="font-family:Orbitron;font-size:0.8rem;color:#c8d8f0;
                         letter-spacing:0.05em;">{p}</div>
                </div>"""
            cards_html += '</div>'
...
with tab6:
    ...
    with col_v1:
        ...
        for feat in features:
            ...
            # label with padding via markdown above each input
            st.markdown(f'<div class="vis-param-label">◈ {feat.upper()}</div>', unsafe_allow_html=True)
...
<style>
  * {{ margin:0; padding:0; box-sizing:border-box; }}
  ...
  /* Normal mode: canvas fills everything minus databar */
  body.normal #c       {{ height:calc(100% - 90px); }}
  body.normal #databar {{ display:flex; }}

  /* Fullscreen mode: canvas fills 100%, databar hidden */
  ...
  #fsBtn {{
    background:rgba(80,20,140,0.75);
    ...
    transition:all 0.2s;
    box-shadow:0 2px 12px rgba(130,50,220,0.35);
  }}
  #fsBtn:hover {{
    background:rgba(130,50,220,0.9); color:#f3e8ff;
    box-shadow:0 2px 20px rgba(192,132,252,0.5);
  }}

  #hint {{
    position:absolute; top:14px; left:14px;
    font-size:0.56rem; color:rgba(167,139,250,0.35);
    letter-spacing:0.07em; pointer-events:none; z-index:10;
  }}
  ...
  #desc {{
    font-size:0.66rem; color:#b39ddb;
  ...
  .dtitle {{
    font-size:0.55rem; color:#6b4fa0;
...
// We resize the iframe itself via window.frameElement so it
// sits over the whole Streamlit page, not just the column.
const NORMAL_H = 660;
  ...
  if (isFull) {{
    ...
    try {{
      if (window.frameElement) {{
        ...
        fe.style.height     = '100vh';   // ← true full viewport height
...
if(TYPE==='STAR'){{
  ...
  [40,28,18,10].forEach((s,i)=>{{
    scene.add(makeGlow(s,mainColor,0.18-i*0.03));
  }});
  scene.add(makeGlow(8,new THREE.Color(1,1,1),0.9));
...
// ══════════════════════════════════════════════════════════════════
  ...
  if(IS_SPIRAL){{
    ...
    for(let arm=0;arm<ARMS;arm++){{
      ...
      for(let p=0;p<2500;p++){{
        ...
        armPts.push(x,y,z);
        const fade=1-pct*0.7,twinkle=0.8+Math.random()*0.2;
    ...
    galaxyGroup.add(new THREE.Points(armGeo,new THREE.PointsMaterial({{
    ...
    const dustGeo=new THREE.BufferGeometry().setFromPoints(
      dustCurve.getPoints(120).map(p=>new THREE.Vector3(p.x,0,p.y)));
    galaxyGroup.add(new THREE.Line(dustGeo,new THREE.LineBasicMaterial({{
    ...
    galaxyGroup.add(new THREE.Points(eGeo,new THREE.PointsMaterial({{
  ...
  [16,10,6,3].forEach((s,i)=>{{
    galaxyGroup.add(makeGlow(s,coreColor,0.55-i*0.1));
  }});
}}
...
// ══════════════════════════════════════════════════════════════════
  ...
  qGroup.add(new THREE.Points(diskGeo,new THREE.PointsMaterial({{
    size:0.4,vertexColors:true,transparent:true,opacity:0.9,
    blending:THREE.AdditiveBlending,depthWrite:false
  }})));

  ['up','down'].forEach(dir=>{{
    const jPts=[],jCols=[],sign=dir==='up'?1:-1;
    for(let i=0;i<3000;i++){{
      const t=Math.pow(Math.random(),0.7);
      const h=sign*(3+t*70),spread=(1-t)*1.8;
      const a=Math.random()*Math.PI*2;
    ...
    qGroup.add(new THREE.Points(jGeo,new THREE.PointsMaterial({{
      size:0.35,vertexColors:true,transparent:true,opacity:0.75,
      blending:THREE.AdditiveBlending,depthWrite:false
    }})));
  }});

  [20,12,7,3].forEach((s,i)=>{{ qGroup.add(makeGlow(s,mainColor,0.6-i*0.1)); }});
  qGroup.add(makeGlow(5,new THREE.Color(1,1,1),0.95));
}}
...
function animate(){{
  ...
  t+=0.008;
  objects.forEach(o=>{{
    if(o.type==='galaxyGroup'||o.type==='qGroup') o.group.rotation.y=t*0.12;
    if(o.type==='star'){{
...
function onResize(){{
  ...
  const nH=canvas.clientHeight||570;
...
onResize();
            ...
            st.components.v1.html(canvas_html, height=660, scrolling=False)
